...
class AnalyticsService:
    async def record_click(
        ...
        print(
            f"\n>>> [ANALYTICS DEBUG] record_click called for '{short_code}' from IP={ip_address}",
            flush=True,
        )
        try:
            ...
            print(
                f">>> [ANALYTICS DEBUG] url found={url is not None}, current count={url.click_count if url else 'N/A'}",
                flush=True,
            )
            if url:
                url.click_count = (url.click_count or 0) + 1
                print(
                    f">>> [ANALYTICS DEBUG] click_count set to {url.click_count}",
                    flush=True,
                )

            await session.commit()
            print(
                f">>> [ANALYTICS DEBUG] commit successful! Final count={url.click_count if url else 'N/A'}",
                flush=True,
            )
            ...
            try:
                ...
                print(
                    f">>> [ANALYTICS ERROR] rollback also failed: {rb_err}", flush=True
                )

    async def get_analytics(
        self, session: AsyncSession, short_code: str
        ...
        country_stmt = (
            select(Analytics.country, func.count(Analytics.id))
        ...
        country_breakdown = {
            row[0] or "Unknown": row[1] for row in country_result.fetchall()
        }

        unique_stmt = select(func.count(Analytics.id)).where(
            and_(Analytics.short_code == short_code, Analytics.is_unique)
        )
        ...
        try:
            ...
            if using_sqlite:
                hourly_stmt = (
                    select(
                        func.strftime("%Y-%m-%dT%H:00:00", Analytics.clicked_at).label(
                            "hour"
                        ),
        ...
        try:
            ...
            if using_sqlite:
                daily_stmt = (
                    ...
                    .where(
                        Analytics.short_code == short_code,
                        Analytics.clicked_at >= month_ago,
                    )
            ...
            daily_trend = [
                {"date": str(row[0])[:10], "clicks": row[1]}
    ...
    def unregister_ws(self, short_code: str, websocket):
        if short_code in _ws_connections:
            _ws_connections[short_code] = [
                ws for ws in _ws_connections[short_code] if ws != websocket
            ]
            if not _ws_connections[short_code]:
                del _ws_connections[short_code]

    async def _notify_ws(
        self, short_code: str, click_event: ClickEvent, total_clicks: int
    ):
        if short_code not in _ws_connections:
            return
        update = LiveClickUpdate(
            short_code=short_code,
            total_clicks=total_clicks,
            latest_click=click_event,
