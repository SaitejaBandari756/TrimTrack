...
class URLSafetyService:
    ...
    def check_apk_malware(self, url: str) -> tuple[bool, str]:
        ...
        if url_lower.endswith(".apk"):
            return (
                False,
                "URL is a direct APK (Android app) download — potential malware risk",
            )
        ...
        try:
            ...
            for i in range(len(domain_parts)):
                ...
                if check_domain in ADULT_DOMAINS:
                    return (
                        False,
                        "⚠️ ADULT CONTENT DETECTED — This URL leads to adult/NSFW website",
                    )
            ...
            for keyword in adult_keywords:
                if keyword.lower() in url_lower:
                    ...
                    return (
                        False,
                        "⚠️ ADULT CONTENT DETECTED — URL contains adult content indicators",
                    )
        except Exception:
            pass
    ...
    def check_suspicious_patterns(self, url: str) -> tuple[bool, str]:
        ...
        redirect_count = (
            url.count("redirect=") + url.count("goto=") + url.count("next=")
        )
        if redirect_count > 3:
            return (
                False,
                "🚨 Suspicious: Multiple redirect parameters detected — potential phishing",
            )
        ...
        for shortener in shortener_domains:
            if url_lower.count(shortener) > 1:
                return (
                    False,
                    "🚨 Suspicious: Chained URL shorteners — potential hidden malware",
                )
    ...
    def check_url(self, url: str) -> tuple[float, bool, str, dict]:
        ...
        if not is_safe:
            return (
                ...
                False,
                reason,
                {"warning_type": "ADULT_CONTENT", "message": reason},
            )
        ...
        if not is_safe:
            return (
                ...
                False,
                reason,
                {"warning_type": "MALICIOUS_PATTERN", "message": reason},
            )
        ...
        if not is_safe:
            return (
                ...
                False,
                reason,
                {"warning_type": "BLOCKED_DOMAIN", "message": reason},
            )
        ...
        if not is_safe:
            ...
            if reason.startswith("🚨"):  # Critical warning
                return (
                    ...
                    False,
                    reason,
                    {"warning_type": "SUSPICIOUS", "message": reason},
                )
        ...
        if ml_score < settings.safety_score_threshold:
            return (
                ml_score,
                False,
                f"ML model flagged URL as suspicious (score: {ml_score:.2f})",
