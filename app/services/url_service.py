...
class URLService:
    async def create_short_url(
        self, session: AsyncSession, request: URLCreateRequest
    ) -> tuple[Optional[URLResponse], Optional[dict]]:
        is_safe, reason = validate_url_safety(request.url)
        if not is_safe:
            return None, {"error": "Invalid URL", "detail": reason, "code": 422}

        safety_score, is_safe, reason, warnings = url_safety_service.check_url(
            request.url
        )
        if not is_safe:
        ...
        url = URL(
            ...
            url_type=request.url_type or "302",
        ...
        await cache_service.set_url_metadata(
            short_code,
            {
                "long_url": request.url,
        ...
        response = URLResponse(
            ...
            created_at=url.created_at or datetime.now(timezone.utc),
            expiry_date=url.expiry_date,
            url_type=url.url_type,
            safety_score=url.safety_score,
            warnings=warnings if warnings else None,
            has_warnings=bool(warnings),
        )
        logger.info(
            f"Created short URL: {response.short_url} using public_base_url: {get_public_base_url()}"
        )
        return response, None

    async def get_url(
        self, session: AsyncSession, short_code: str
    ) -> tuple[Optional[URL], Optional[dict]]:
        cached_meta = await cache_service.get_url_metadata(short_code)
        if cached_meta:
            if not cached_meta.get("is_active", True):
                return None, {
                    "error": "Gone",
                    "detail": "This URL has been deleted",
                    "code": 410,
                }
        ...
        if cached_url and cached_meta:
            url_obj = URL(
                short_code=short_code,
                long_url=cached_url,
                url_type=cached_meta.get("url_type", "302"),
                is_active=True,
            )
            return url_obj, None
        ...
        if not url:
            return None, {
                "error": "Not Found",
                "detail": f"Short code '{short_code}' does not exist",
                "code": 404,
            }
        ...
        if url.expiry_date and url.expiry_date.replace(
            tzinfo=timezone.utc
